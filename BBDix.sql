...
select code_cat from trophee where num_resserre = 3;
...
select num_trophee from trophee t where date_prise > '2052-05-01' and date_prise < '2052-06-30' ;
...
select distinct num_hab from absorber a where num_potion in (1,3,4);

--7. Liste des trophées : numéro, date de prise, nom de la catégorie et nom du preneur. (10lignes)
select t.num_trophee, t.date_prise , categorie.nom_categ , habitant.nom 
from trophee t 
inner join categorie on t.code_cat = categorie.code_cat 
inner join habitant on t.num_preneur = habitant.num_hab ;
...
select count (h.num_hab) as nombre_habitant_village_5
from habitant h 
...
select v.nom_village, count(h.num_hab) as nombre_habitants_par_village
from habitant h 
inner join village v on h.num_village  = v.num_village 
...
select h.nom, count(distinct p.num_potion) as nombre_de_potions_differente 
from habitant h
inner join absorber a on h.num_hab = a.num_hab 
inner join potion p on a.num_potion = p.num_potion 
group by h.nom;
